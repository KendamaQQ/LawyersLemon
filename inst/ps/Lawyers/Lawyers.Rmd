...
<br>
- Tasks: They describe, what is getting done in the succeeding code chunck. Sometimes the code is already proided, in this circumstance, you will be asked just to press `check` and continue. 
...
The two main variables, that we are going to use in our analysis are `guilty` and `appointed`. Both of them take only values between 0 and 1. In the case of `guilty`, 0 indicates, that the defendant wasn't convicted and 1 stands for conviction. In the case of `appointed`, 0 indicates, that the attorney was retained by a defendant and 1 means the attorney was assigned to a indigent defendant. Let's take a look at both variables. 
...
**Task:**
Use the `mean()` function along with `appointed` and `guilty` to compute the assignment and conviction rate. The code for the the assignment rate is already provided, so you just have to compute the `mean()` of `guilty`.
...
The result reveals that 63 % of fellony cases in Bexar County between 2005 and 2014 were negotiated by assigned counsels and 48 % of clients were convicted. To know how many people were convicted overall isn't very informative, if we are interested in studying differences between two groups (retained vs. assigned). Before we compute the conviction rate based on counsel appointment, try to guess how big the gap in conviction rates between retained and assigned counsels is.
...
As you see, there are 23559 cases in our data with an attorney being retained and 40650 times an indigent defendant got a counsel assigned. But more interesting is that the conviction rate obviously differs in both groups. Defendants with an assigned attorney are **18.3 percentage points** more likely to be found guilty. In this context it's important to distinguish between a percent change and a percentage point change. We could also state the result as a percent change: Defendants with an assigned attorney are **50.3 %** more likely to be found guilty. By looking at the numbers, it's easy to realise, that there is a huge difference. Both conviction rates are stated in percentage points, so the spread between them is also stated in percentage points. However, if we divide this difference by the conviction rate of defendants with a retained counsel we have computed the change in percent. Note, that it is also possible to divide the percentage point difference by the conviction rate of defendants with an assigned attorney. Obviously, we would get another solution, which emphasises the importance to be clear, which value is used in the denominator.   
...
As it can be seen, the attorney in question was assigned in 94% of all cases he resolved. Are attorneys, who resolved more cases more often assigned in general? And if so, why? One reason might be, that they handle cases, which are simpler to resolve. But another explenation could be, that they exert less effort if assigned and hence resolve cases quick but at the expense of their clients. We won't answer this question now, but if you keep going you will encounter the answers.   
...
The variable `exp.fig` will be the x variable in our plot. But we are still necessitate a y variable. Unfortunately, this one isn't so easy to determine, but we already acquired the knowledge to solve the tasks.   
The approach is first to group by the attorneys and their experience to compute the mean appointed rate and than to group by the experience only and compute the overall fraction of cases assigned per year of experience. 
...
After we get some insights into the fundamentals of R as well as the paper we will now continue to dive deeper into both. In exercise 2.1 we will run our first regression analysis. If you are already familiar with OLS that shouldn't be something, that scares you. If you are not, you will learn the basics in the exercise. Additionally, after you finished exercise 2.1 you will now, what the authors mean, when they talk about the (unconditional) assigned counsel penalty and how to estimate it. We will also discover that case characteristics, adverse selection, matching preferences and moral hazard are potential explenations for the unconditional assigned counsel penalty. Exercise 2.1 also provides a introduction of potential variables, that account for case characteristics. However, in exercise 2.2 we will run the model proposed by authors to control for all case and attorney characteristics. 
...
**Task:**
Run a linear model with `lm()`. Use `guilty` as dependant and `appointed` as independant variable. Finally use `summary()` to produce the output table of the regression results.
...
Note, that we call the coefficient of `appointed` the (unconditional) **assigned counsel penalty**. The "unconditional" simply refers to the fact, that we don't have incorporated further explenatory variables besides `appointed` yet. 
...
#! addon__quiz__other outcomes unconditional
...
#! addon__quiz__case characteristics
...
**Task:**
Run a linear regression model with `lm()`. Use `guilty` as dependant variable and `hadbondsman` as the only regressor. 
...
The result shows, that posting a bail decreases the likelihood of being found guilty by 35.7% on average. Now, let's see, what happends to the assigned counsel penalty, if we add `hadbondsman` to our linear regression model from equation (1). 

**Task:**
Use `lm()` to run a linear regression model. Again, use `guilty` as dependant variable and `appointed` plus `hadbondsman` as independant variables. 
...
**Task:**
Firstly, use `lm()` to run a regression with `appointed` as dependant variable and `hadbondsman` as only regressor. Save the regression in a variable `lm.3`. Because the `lm()` function returns a object of class `lm`, we can access different regression outputs with the help of `$`, e.g. `lm.3$coefficients` retrieves just the coefficients from the regression.   
Secondly, use `lm()` again to run a regression with `guilty` as dependant variable and use the residuals from `lm.3` as independant variable. Note, that all you have to do in order to incorporate the residuals into the model is to retrieve it by `lm.3$residuals`.   
...
#! addon__quiz__all relevant variables
...
**Task:**
Use `lm()` again to run a regression with `guilty` as dependant and `appointed`, `hadbondsman` and `male` as independant variables. Save the regression in a variable `lm.5` and show the result with `summary()`.
...
As you can see, adding `male` again changed all coefficients. However, this time the assigned counsel penalty didn't declined further but increased by 1 percentage point again. Why? There are two potential sources. On the one hand, there is the direct relationship between `male` and `appointed`. On the other hand, there is also the indirect influence of `male` on `appointed` through `hadbondsman`. We have already seen before, that `hadbondsman` has an severe effect on the assigned counsel penalty. If we look at the coefficient of `hadbondsman` after adding `male` it can be seen, that the coefficient decreased by 2.6 percentage points. The reduced magnitude of `hadbondsman` therefore impacts the assigned counsel penalty less strongly, which means, that the coefficient of `appointed` is a little bit higher, as we have seen.   

Of course we could and should add further variables to our model in order to account for case characteristics. However, we will see, that sequential addition of covariates is problematic in exercise 3.1 and hence doesn't really help to assess the influence of individuel case characteristics on `appointed`. Therefore we will investigate the full set of case characteristic variables in the next exercise along with adverse selection. What you should realized in this exercise is that `appointed` is sensitive to other variables, that can help us to account for case characteristics and adverse selection.
...
As a last step, before running the model, let's discuss attorney-by-year fixed effects $\gamma_{kt}$. This are dummy variables for every attorney-year combination, that we include into our model. We do so, because we want to account for **unobserved heterogenity**. That means, that there is variation in variables, that could help us to better assess the influence of `appointed` on `guilty`, but we can't observe this variables (we don't have them in our data). Including attorney-by-year fixed effects solves this problem, because it accounts for any unobserved variation in attorneys across years. In this specification, the assigned counsel penalty is identified off variation within attorneys who work as both assigned and retained counsel in the same year. To get a better grasp about the various dummys of attonrey-by-year fixed effects, let's compute how many are present and the number of cases in each group. 
...
Finally, let's contemplate the other outcome variables, we have put aside until now. Everything we discussed so far applies just as well for this other variables. We will take a look at `dismissed`, `defadj`, `incarcerated`, `reduced`, `lnsent` and `lnfine`.
...
In this section you will learn, how we can decompose the difference between the conditional and unconditional aissgned counsel penalty in exercise 3.1. Afterwards, we will thoroughly examine matching preferences and moral hazard, which are two possible explenations for the residual assigned counsel penalty.
...
**Task:**
Run for both added variables `complaint_hist` and `hadbondsman` two regressions. Regress them on `appointed` in the first one and then add the other as explenatory variable in the second.  
...
\end{align}$$
as the OLS estimated coefficient on $X_1$ from an auxiliary model with $X_{2k}$ as the dependant variable. Because the omitted variables bias formula is linear we can restate equation (5) as
$$\begin{align}
...
info("preknit_XxHbQLWjmTud") # Run this line (Strg-Enter) to show info
...
info("preknit_bZkFsIxEBaNF") # Run this line (Strg-Enter) to show info
...
info("preknit_IzqOIsGhQBGx") # Run this line (Strg-Enter) to show info
...
info("preknit_OJTqblcklTov") # Run this line (Strg-Enter) to show info
...
info("preknit_hvsIaKjwnFKD") # Run this line (Strg-Enter) to show info
...
After we defined the function let's first reassure ourselfs, that we did everything correct. So before running the model with full specifications, let's test the function on the simple model in equation (4). 
...
**Task:**
Use the newly defined function `gelbach.decomposition()` to decompose the model stated in (2) in relation to the base model with `appointed` being the only explenatory variable. 
...
info("preknit_dZVQxyTcUiWU") # Run this line (Strg-Enter) to show info
...
For representation purposes we will just focus on `guilty` again and begin with a regression where we add an interaction term for the variable `attymale`, which indicates the attorneys gender. Note that the authors also include the new scaled `reg_exp` as additional explenatory variable, so we will do the same. 
...
**Task:**
Use the `felm()` function with the familiar set of regressors from equation (2) and the dependant variable `case_length`. 
...
**Task - Step 3:**
Now run a regression with `recidicism3` as dependant variable, which takes the value 1, if the defendant gets charged with a felony again within three years and 0 else. The explenatory variables are provided in the chunck. Note, that we speficy the `subset` argument of the `lm()` function such that only defendants who hasn't been incarcerated get considered in the regression.
...
**Task - Step 5:**
In this chunck the mean residuals get averaged for each attorney. Further, we compute the assignment rate for each attorney and append the solution to our data with `mutate()`. 
Just press `check`. 
...
The dependant variable is the vector of mean residuals for assigned cases `mean_resid_appt` and the independant variable is the vector of attorney-specific residual assigned counsel penaltys, `attybeta`, calculated in the steps 1 and 2. 
